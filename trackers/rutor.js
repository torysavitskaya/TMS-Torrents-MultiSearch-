// ==UserScript==
// @name Рутор
// @description Открытый торрент трекер.
// @icon data:image/x-icon;base64,
// @downloadURL https://github.com/Feverqwe/tSearch/raw/master/src/trackers/rutor.js
// @trackerURL http://rutor.info
// @version 1.0.4
// @connect *://rutor.info
// @require exKit
// ==/UserScript==

const code = {
  "version": 3,
  "type": "kit",
  "description": {
    "icon": "data:image/x-icon;base64,",
    "name": "Рутор",
    "description": "Открытый торрент трекер.",
    "downloadUrl": "https://github.com/Feverqwe/tSearch/raw/master/src/trackers/rutor.js",
    "version": "1.0.4"
  },
  "search": {
    "url": "http://rutor.info/search/0/0/100/0/%search%",
    "method": "GET"
  },
  "selectors": {
    "row": {"selector": "#index > table > tbody > tr"},
    "skipFromStart": 1,
    "title": {
      "selector": "td:eq(1)>a:eq(-1)",
      "pipeline": [{"name": "getText"}]
    },
    "url": {
      "selector": "td:eq(1)>a:eq(-1)",
      "pipeline": [{"name": "getProp", "args": ["href"]}]
    },
    "size": {
      "selector": "td:eq(-2)",
      "pipeline": [{"name": "getText"}, {"name": "parseSize"}]
    },
    "downloadUrl": {
      "selector": "td:eq(1)>a:eq(-2)",
      "pipeline": [{"name": "getProp", "args": ["href"]}]
    },
    "seeds": {
      "selector": "td:eq(-1)>span.green",
      "pipeline": [{"name": "getText"}, {"name": "toInt"}]
    },
    "peers": {
      "selector": "td:eq(-1)>span.red",
      "pipeline": [{"name": "getText"}, {"name": "toInt"}]
    },
    "date": {
      "selector": "td:eq(0)",
      "pipeline": [
        {"name": "getText"},
        {"name": "legacyReplaceMonth"},
        {"name": "legacyParseDate", "args": ["1"]}
      ]
    },
    "nextPageUrl": {"selector": "#index > table ~ b"}
  }
};

code.hooks = {};
code.hooks.onBeforeRequest = (session, options) => {
  if (session.event === 'search' && !session.request.query) {
    options.url = 'http://rutor.info/browse/';
  }
};
code.hooks.transform = {};
code.hooks.transform.nextPageUrl = (session, result, container) => {
  let url = null;
  let currentPage = null;
  result && [].slice.call(result.childNodes).some((node) => {
    let index = null;
    const m = /\s*(\d+)\s*$/.exec(node.textContent);
    if (m) {
      index = parseInt(m[1], 10);
    }
    if (index !== null && node.nodeType === 3) {
      currentPage = index;
    }
    if (currentPage !== null && index === currentPage + 1 && node.tagName === 'A') {
      url = node.href;
      return true;
    }
  });
  return url;
};

API_exKit(code);